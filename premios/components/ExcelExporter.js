 ...
 * Copia de /bi/global/components/ExcelExporter.js (convención del repo: se
 * duplica por módulo en lugar de importarse).
...
const ExcelExporter = (() => {
    ...
    function _getContext() {
        ...
        const selSupervisora = document.getElementById('sel-supervisora');
        const supText = selSupervisora?.options[selSupervisora.selectedIndex]?.text ?? '';

        const parts = [];
        if (supText && supText !== 'Todas') parts.push('Supervisora: ' + supText);
